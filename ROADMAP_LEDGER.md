...
- [ ] Add one private workspace authority for project root, configuration,
  route-source ownership, URI/path normalization, symlink policy, and generated
  output ownership.
- [ ] Derive real route graph definitions, structured diagnostics, explanations,
  and complete generated bytes from one analyzer operation without writing files.
- [ ] Prove canonical-app success, collision refusal, correction guidance, flow
  inspection, and recovery from that operation without a second policy path.
- [ ] Keep package exports, command surface, analyzer schema, and editor products
