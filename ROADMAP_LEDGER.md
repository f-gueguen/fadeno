...
V1-DX-C4 — packed cancellation, supersession, and cleanup qualification

## Exit criteria

- [ ] Use deterministic barriers to prove active packed analyzer cancellation
  and supersession without timing sleeps or retry-selected evidence.
- [ ] Prove an obsolete completed operation cannot emit a consumer event or
  overwrite the newest accepted diagnostic and artifact generation.
- [ ] Close during active work and prove operation, observer, transaction,
  generated-artifact, compiler, and disposable-consumer ownership settles to
  zero without overclaiming operating-system or editor-product behavior.
- [ ] Add permanent normalized interruption, refusal, flow, recovery, and
  cleanup evidence derived only from delivered events and explicit barriers.
- [ ] Pass lifecycle, package, exact local CI, architecture, Big-O, and fresh
  independent review gates without adding an export, command, protocol, editor
  metadata, stable machine schema, or supported editor product.

## In progress

- V1-DX-C3 completed at implementation commit `d4b3275`: a current packed
  private consumer observes only filesystem-adapter deliveries; proves direct
  and genuine three-edge imported freshness, configuration epochs, rename and
  deletion cleanup, all seven generated bytes, diagnostic refusal versus
  last-good disk state, and exact recovery; and retains permanent normalized
  success, refusal, flow, stale-artifact, recovery, and human diagnostic
  evidence.
- C4 now qualifies interruption and cleanup at that same packed private event
  boundary. It must use injected deterministic barriers to make active work,
  cancellation, supersession, obsolete completion, close, and cleanup exact.
- Existing coordinator, publication, provisional application, compiler, and
  watcher cancellation remain authoritative. C4 observes and qualifies their
  composition; it does not create a second lifecycle or cancellation policy.
- C4 must distinguish cancellation from supersession, prove that only the
  newest complete generation is delivered, and account for every retained
  private owner after close. Timing sleeps, internal-current reads, garbage
  collection as correctness, and process-exit cleanup are not evidence.
- Complete feedback workload freezing and measurement remain C5A and C5B.
  The analyzer remains private and tool-neutral; C4 adds no command, generic
  protocol, package export, editor metadata, stable external schema, or
