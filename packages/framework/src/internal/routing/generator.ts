...
function renderApplication(manifest: RouteManifest): string {
  ...
  const defaultRoute = [...manifest.routes]
    .filter((route) => route.notFound !== null)
    .sort((left, right) => left.segments.length - right.segments.length || compareText(left.id, right.id))[0];
  const defaultNotFound = defaultRoute?.notFound ? bindings.get(defaultRoute.notFound) : undefined;
  const defaultLayouts = defaultRoute?.layouts.map((path) => bindings.get(path)).join(", ") ?? "";
  const defaultError = defaultRoute?.error ? bindings.get(defaultRoute.error) : undefined;
  return [
    ...
    "  const selected = match(new URL(request.url).pathname);",
    `  if (!selected) return renderRoute({ request, parameters: Object.freeze(Object.create(null) as Record<string, never>), page: () => notFound(), layouts: [${defaultLayouts}], notFound: ${defaultNotFound ?? "undefined"}, error: ${defaultError ?? "undefined"} });`,
    "  switch (selected.id) {",
    ...cases,
