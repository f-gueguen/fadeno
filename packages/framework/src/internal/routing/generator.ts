...
function renderApplication(manifest: RouteManifest): string {
  ...
  const defaultNotFoundPath = manifest.routes
    .map((route) => route.notFound)
    .filter((value): value is string => value !== null)
    .sort((left, right) => left.split("/").length - right.split("/").length || compareText(left, right))[0];
  const defaultNotFound = defaultNotFoundPath ? bindings.get(defaultNotFoundPath) : undefined;
  return [
    ...
    `  if (!selected) return renderRoute({ request, parameters: Object.freeze(Object.create(null) as Record<string, never>), page: () => notFound(), layouts: [], notFound: ${defaultNotFound ?? "undefined"} });`,
    "  switch (selected.id) {",
    ...cases,
