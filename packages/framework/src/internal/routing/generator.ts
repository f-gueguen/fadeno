...
export function decodeRouteArtifactManifest(bytes: string): RouteManifest {
  let value: unknown;
...
function renderApplication(manifest: RouteManifest): string {
  ...
  const cases = manifest.routes.map((route) => {
    ...
    const boundaries = `${notFound ? `, notFound: ${notFound}` : ""}${error ? `, error: ${error}` : ""}`;
    return `    case ${JSON.stringify(route.id)}: return renderRoute({ request, routeId: selected.id, generation: applicationGeneration, parameters: selected.parameters, page: ${source}, layouts: [${layouts}]${boundaries} });`;
  });
  ...
  const fallbackCases = fallbacks.map((fallback) => {
    const error = fallback.error ? `, error: ${bindings.get(fallback.error)}` : "";
    return `    case ${JSON.stringify(fallback.id)}: return renderRoute({ request, routeId: selectedFallback.id, generation: applicationGeneration, parameters: selectedFallback.parameters, page: () => notFound(), layouts: [${fallback.layouts.map((path) => bindings.get(path)).join(", ")}], notFound: ${bindings.get(fallback.source)}${error} });`;
  });
  return [
    ...
    'import { notFound, renderRoute, type Handler } from "@fadeno/framework";',
...
function assertOwnedFiles(files: Readonly<Record<string, string>>): RouteManifest {
  ...
  const manifest = decodeRouteArtifactManifest(files["manifest.json"]!);
