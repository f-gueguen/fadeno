...
| DG-V1-06 | Public `fadeno build` or `fadeno dev` implementation | Define production entry and compiler inputs, transactional `dist` layout, documented start command, development server/address/readiness, watch and restart behavior, diagnostics and last-good policy, shutdown, streams, and exit statuses | Retained coordinator/compiler pipeline evidence plus packed canonical application build and change scenarios | Build/development command and lifecycle ADR | Open |
