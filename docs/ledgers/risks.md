...
| Analyzer consumers observe stale or mixed generations | ADR 0030 requires versioned documents, workspace epochs, complete affected closure, atomic replacement, cancellation, and stale-result suppression; B7D1 serializes retained analysis/explanation admission and invalidates older derived capabilities | Diagnostics, declarations, manifests, mappings, or deletions from different epochs become jointly observable |
...
| Retained analysis accumulates historical route-root source text | B7D1 has no watch consumer and explicit close bounds its current use; B7D2 must batch-reconcile and forget closed unmanaged document owners before retained watch/build consumption | Repeated disjoint route-root changes make retained memory or reconciliation cost depend on cumulative historical files rather than current ownership |
