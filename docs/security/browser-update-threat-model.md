...
| Browser restoration races runtime focus or scroll | Runtime temporarily selects manual restoration, focuses with scroll prevention, commits new links at the top, and restores the prior setting on close, startup refusal, or native recovery reload | Focus/scroll ordering, reload/restart, and teardown checks |
...
| Scroll changes race startup, traversal, or commit | Startup restores native scroll ownership after acquisition failure; displayed entry identity changes only after commit; bounded unsafe tracking fails closed on overflow; exact selected URL/state is revalidated before commit; pre-interception and pre-commit flushes capture late document scroll | Startup, overflow, state-replacement, multi-traversal, same-task traversal, and late-scroll recovery checks |
