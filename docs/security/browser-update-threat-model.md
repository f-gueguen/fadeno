...
| History selection succeeds before a later document commit, provenance check, or rollback failure | Document and history postconditions share one rollback boundary; a pushed selection rolls back before native reselection while a traversal replacement stays selected, preserving one-Back access without appending a duplicate destination | Three-engine focus/scroll commit failure, focus-time history mutation, persistent rollback failure, and single-entry recovery |
