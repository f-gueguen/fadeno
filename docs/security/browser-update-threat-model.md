...
| Pending form ownership leaks or changes submitted meaning | Only the exact form receives `aria-busy`; controls are classified by their actual form owner rather than DOM containment, are not disabled or rewritten, and prior state is restored on success, expected failure, redirect, refusal, cancellation, recovery, close, and commit rollback | Pending visibility, external-owner dirty-control refusal, successful-control equality, duplicate suppression, and terminal cleanup cases |
