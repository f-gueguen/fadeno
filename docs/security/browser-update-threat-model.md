...
| Scroll changes race startup, traversal, or commit | Startup restores native scroll ownership after acquisition failure; displayed entry identity changes only after commit; bounded unsafe tracking and a bounded chain-scoped restart record fail closed; scroll during pending traversal marks the displayed owner; exact selected URL/state is revalidated before commit; pre-interception and pre-commit flushes capture late document scroll | Startup, overflow, state-replacement, multi-traversal, same-task, pending-traversal, and late-scroll recovery checks |
