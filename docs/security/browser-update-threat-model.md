...
| Scroll changes race startup, traversal, or commit | Startup restores native scroll ownership after acquisition failure; bounded entry identity marks the outgoing document before adopting a traversal; pre-interception and pre-commit flushes capture late document scroll | Startup, same-task traversal, and late-scroll recovery checks |
