...
| History state is malformed, application-owned, or from another runtime chain | Only a state freshly created or re-keyed by the current document runtime, with the exact private marker/version, active session identity, bounded entry/URL recovery record, and bounded scroll record, is interpreted; guarded runtime writes are distinguished from application History API calls, and every other selected entry reloads current URL truth | Three-engine malformed/foreign/unowned traversal, exact-shape startup re-keying, and same-URL copy recovery before and after reload |
| Application code copies a private-looking history state | A bounded active-runtime registry requires the exact state and URL created by the runtime, rejects repeated selection without an observed runtime traversal, persists the application-owned result across reload and every restart in that document, gives every actual restart a fresh session and entry while retaining same-document unsafe-scroll evidence, applies one 8,192-byte URL class to persistence reads and writes, and fails closed on overflow | Three-engine cloned-entry, repeated-reload and post-close re-keying, long-URL persistence, and same-URL application-copy recovery |
| Browser restoration races runtime focus or scroll | Runtime temporarily selects manual restoration, guards History-wrapper installation, restores any partially installed wrapper, focuses with scroll prevention, creates the destination entry before resetting its viewport, verifies the actual viewport reached the top, restores exact prior focus on document rollback, and restores the prior setting on close, page departure, startup refusal, or native recovery reload | Wrapper-installation refusal, focus/scroll ordering and postcondition, rollback-focus, native-departure, reload/restart, and teardown checks |
...
| A refused same-context activation or close occurs while work is pending | Eligible and refused links plus still-native form submissions abort any pending enhanced request; traversal ownership is repaired before native activation continues, ordinary close aborts its pending link request before teardown, and form submission is not intercepted | Three-engine eligible-click, pending ordinary-request fragment-link, traversal fragment-link, native-form, ordinary-close, and delayed-recovery supersession with stale-document suppression |
