...
| Application code copies a private-looking history state | A bounded active-runtime registry requires the exact state and URL created by the runtime, rejects repeated selection without an observed runtime traversal, persists the application-owned result across reload and every restart in that document, re-keys a later loaded document before resumption, applies one 8,192-byte URL class to persistence reads and writes, and fails closed on overflow | Three-engine cloned-entry, repeated-reload re-keying, long-URL persistence, and same-URL application-copy recovery |
| Browser restoration races runtime focus or scroll | Runtime temporarily selects manual restoration, focuses with scroll prevention, creates the destination entry before resetting its viewport, verifies the actual viewport reached the top, restores exact prior focus on document rollback, and restores the prior setting on close, page departure, startup refusal, or native recovery reload | Focus/scroll ordering and postcondition, rollback-focus, native-departure, reload/restart, and teardown checks |
...
| A refused same-context activation or close occurs while work is pending | Eligible and refused links plus still-native form submissions abort the traversal and repair displayed truth before native activation continues; ordinary close aborts its pending link request before teardown; form submission is not intercepted | Three-engine eligible-click, fragment-link, native-form, ordinary-close, and delayed-recovery supersession with stale-document suppression |
