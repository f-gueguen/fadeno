...
| A user cancels the reload required by an unsafe traversal | The still-active document repairs the selected slot to a fresh private entry at its trusted displayed URL, reacquires manual restoration, records the refusal, and resumes enhancement | Three-engine cancelled-reload URL/document repair, flow, and resumed-navigation recovery |
