...
| Application code copies a private-looking history state | A bounded active-runtime registry requires the exact state and URL created by the runtime, rejects repeated selection without an observed runtime traversal, and fails closed on overflow | Three-engine cloned-entry back/forward recovery |
...
| Scroll changes race startup, traversal, close, or commit | Startup restores native scroll ownership after acquisition failure; displayed entry identity changes only after commit; bounded unsafe tracking and a bounded chain-scoped restart record fail closed; scroll during pending traversal marks the displayed owner, cancels pending work, and reloads selected truth; recorded element-scroll ownership keeps later links native; close during traversal reloads the selected URL; exact selected URL/state is revalidated before commit; pre-interception and pre-commit flushes capture late document scroll | Startup, overflow, state-replacement, element-link refusal, multi-traversal, same-task, pending-traversal interruption, close-traversal, and late-scroll recovery checks |
| History selection succeeds before a later document commit or rollback failure | Native recovery preserves the already-selected classification and replaces that destination instead of appending it, preserving one-Back access to the prior document | Three-engine focus/scroll commit failure, persistent rollback failure, and single-entry recovery |
...
| A newer click occurs while traversal work is pending | The click aborts the traversal before remaining native and observes canceled departure so obsolete work cannot publish or leave selected history paired with the displayed document | Three-engine native-click supersession and stale-document suppression |
| A user cancels required native recovery | Both preventDefault and legacy returnValue confirmation are observed; the still-active document repairs the selected slot to a fresh private entry at its trusted displayed URL, reacquires manual restoration, records the refusal, and resumes enhancement | Three-engine traversal reload and post-selection fallback cancellation repair, flow, and resumed-navigation recovery |
