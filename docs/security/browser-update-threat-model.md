...
| Pending form ownership leaks or changes submitted meaning | Only the exact form receives `aria-busy`; controls are not disabled or rewritten; prior state is restored on success, expected failure, redirect, refusal, cancellation, recovery, close, and commit rollback | Pending visibility, successful-control equality, duplicate suppression, and terminal cleanup cases |
| An uncertain mutation is repeated | Mutation is committed for recovery when fetch begins; every later transport, decode, projection, cancellation, close, or commit failure performs a trusted GET current-truth reload and never invokes POST again | Before/during/after-request interruption, duplicate, close, malformed response, projection refusal, rollback, and POST-count assertions |
