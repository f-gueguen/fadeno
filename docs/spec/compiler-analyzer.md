...
diagnostic batch from that same operation even though artifact application is
not accepted. Their operation identities must match. This lets a consumer
observe current desired analyzer state separately from the prior accepted disk
generation without consulting session current state or rebuilding analyzer
policy. Successful filesystem-adapter callbacks synchronously copy every
delivered generated value and the corresponding applied bytes; failure
callbacks copy the last-good bytes. Observer exceptions remain isolated and
therefore cannot stand in for assertions made after the awaited delivery.

The transitive control has one configured root input importing a three-edge
chain whose deepest dependency is outside the root-file include. A leaf-only
saved edit must change the delivered stock-compiler input identity while the
framework publication and generated bytes remain identical. A comment-only
configuration edit is the sole mutation in its cycle and must advance the
configuration epoch without artifact churn. Rename, deletion, refusal, and
repair must account for all seven generated route artifacts, exact declaration
and manifest contents, disappeared source ownership, transaction debris, and
the difference between current refused desired state and preserved last-good
