...
const loopback = createDecisionSession(keyring, {}, now, "loopback-http:3001");
assert.equal(openDecisionSession(keyring, loopback.envelope, now, "loopback-http:3001").status, "valid");
assert.equal(openDecisionSession(keyring, loopback.envelope, now, "loopback-http:3002").status, "invalid");
assert.equal(openDecisionSession(keyring, loopback.envelope, now).status, "invalid");
