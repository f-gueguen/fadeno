...
function projectSources(projectRoot: string, generation: number, handlerValue: string, invalid = false): void {
  ...
  writeFileSync(join(projectRoot, "server/bootstrap.ts"), [
    'import { generation } from "fadeno:routes";',
    'import { handlerValue } from "../src/handler.ts";',
...
try {
  ...
  const packageIdentity = decision.capturePrivateRuntimeIdentity(installedPackage, [
    "package.json",
    "dist/internal/build-dev-decision.js",
    "dist/internal/build-dev-generation-child.js",
  ]);
  ...
  const typescriptIdentity = decision.capturePrivateRuntimeIdentity(typescriptRoot, [
    "package.json", "bin/tsc", "lib/getExePath.js", "lib/tsc.js", "dist/api/sync/api.js",
  ]);
  ...
  const nativeIdentity = decision.capturePrivateRuntimeIdentity(nativeRoot, ["package.json", "lib/tsc"]);
  ...
  projectSources(projectRoot, 1, "one");
  ...
  projectSources(projectRoot, 2, "two");
  ...
  projectSources(projectRoot, 3, "invalid", true);
  ...
  projectSources(projectRoot, 4, "three");
