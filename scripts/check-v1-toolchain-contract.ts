...
try {
  ...
  try { await executePrototype(invalid, "check"); } catch (error: unknown) { invalidRejected = error instanceof Error && error.message === "FADENO_CONFIG_EXPORTS"; }
  ...
  try { await executePrototype(unknown, "check"); } catch (error: unknown) { unknownRejected = error instanceof Error && error.message === "FADENO_CONFIG_ROUTES"; }
