...
try {
  ...
  const expectedEntries = [
    ...
    "package/dist/internal/action.d.ts",
    ...
    "package/dist/internal/server-update-transport.d.ts",
    "package/dist/internal/server-update-transport.js",
  ...
  const analyzerCoordinatorJs = join(installedPackage, "dist/internal/analyzer-coordinator.js");
  const analyzerCoordinatorTypes = join(installedPackage, "dist/internal/analyzer-coordinator.d.ts");
  if (!existsSync(analyzerCoordinatorJs) || !existsSync(analyzerCoordinatorTypes)) throw new Error("FADENO_PUBLIC_PACKAGE_ANALYZER_COORDINATOR_INTERNAL_ABSENT");
  ...
  const analyzerWatcherJs = join(installedPackage, "dist/internal/analyzer-watcher.js");
  const analyzerWatcherTypes = join(installedPackage, "dist/internal/analyzer-watcher.d.ts");
  if (!existsSync(analyzerWatcherJs) || !existsSync(analyzerWatcherTypes)) throw new Error("FADENO_PUBLIC_PACKAGE_ANALYZER_WATCHER_INTERNAL_ABSENT");
