...
expectMutation("A0 alpha qualification is not bound to its qualified seed source", (context) => Object.freeze({
  ...context,
  packageManifest: { ...(context.packageManifest as Record<string, unknown>), version: "0.1.0-alpha.0" },
}));
expectMutation("A0 alpha package identity or publication boundary drifted", (context) => {
  const document = cloneDocument();
