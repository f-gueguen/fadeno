...
export const V2_PLAN_ROWS = Object.freeze([
  ...
  { id: "V2-10", features: ["PERF-01", "TEST-01", "DOC-01"], dependencies: "V2-09", artifacts: "Runtime byte and execution-cost records naming exact source commit, packed-package digest, measured browser-artifact digest, dataset/corpus identity, hardware and environment, runtime/tool versions, exact command, warmups, repetitions and schedule, raw per-sample results, conclusion, and limitations; explicit no-release-impact declaration for measurement-only work", validation: "Reproducible cost-record schema and identity mutation checks; independent source/package/browser-artifact identity verification and command replay over retained raw samples; conclusion/limitation checks; release-impact check; `pnpm ci:local`" },
...
export function validateV2Plan(context: V2PlanContext): readonly string[] {
  ...
  if (JSON.stringify(ids) !== JSON.stringify(expectedIds)) errors.push("V2 roadmap slices must be exactly V2-00, V2-01, V2-01A, V2-02 through V2-10, V2-10A, V2-10B, V2-11, then V2-12 in order");
