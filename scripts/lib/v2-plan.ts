...
export const V2_PLAN_ROWS = Object.freeze([
  ...
  { id: "V2-06", features: ["DATA-02", "ENH-01", "STATE-01", "SEC-01", "TEST-01"], dependencies: "V2-03, V2-04", artifacts: "Form interception boundary; exact successful controls; pending ownership; expected validation; uncertain-mutation refusal; teardown; one pending Changeset with semantic version intent", validation: "Native/enhanced payload and outcome equivalence; duplicate/replay/cancel/network failure; safe fallback and recovery; Changeset checks; `pnpm ci:local`" },
  ...
  { id: "V2-10", features: ["PERF-01", "TEST-01", "DOC-01"], dependencies: "V2-09", artifacts: "Runtime byte and execution-cost records naming dataset/corpus identity, hardware and environment, runtime/tool versions, exact command, warmups, repetitions and schedule, raw per-sample results, conclusion, and limitations; explicit no-release-impact declaration for measurement-only work", validation: "Reproducible cost-record schema and mutation checks; independent command replay over retained raw samples; conclusion/limitation checks; release-impact check; `pnpm ci:local`" },
...
export function validateV2Plan(context: V2PlanContext): readonly string[] {
  ...
  if (JSON.stringify(ids) !== JSON.stringify(expectedIds)) errors.push("V2 roadmap slices must be exactly V2-00, V2-01, V2-01A, V2-02 through V2-10, V2-10A, V2-10B, then V2-11 in order");
